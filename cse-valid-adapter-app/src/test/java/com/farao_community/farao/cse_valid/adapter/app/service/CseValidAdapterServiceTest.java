...
class CseValidAdapterServiceTest {
    ...
    @Test
    void runAsyncShouldNotRunTaskBecauseTaskStatusIsCreated() {
        TaskDto taskDto = TestData.getTaskDto(TaskStatus.CREATED);
        cseValidAdapterService.runAsync(taskDto);
        verify(cseValidClient, never()).run(any());
        verify(streamBridge, never()).send(any(), any());
    }
    ...
    @Test
    void runAsyncShouldNotRunTaskBecauseTaskStatusIsPending() {
        TaskDto taskDto = TestData.getTaskDto(TaskStatus.PENDING);
        cseValidAdapterService.runAsync(taskDto);
        verify(cseValidClient, never()).run(any());
        verify(streamBridge, never()).send(any(), any());
    }
    ...
    @Test
    void runAsyncShouldNotRunTaskBecauseTaskStatusIsRunning() {
        TaskDto taskDto = TestData.getTaskDto(TaskStatus.RUNNING);
        cseValidAdapterService.runAsync(taskDto);
        verify(cseValidClient, never()).run(any());
        verify(streamBridge, never()).send(any(), any());
    }
    ...
    @Test
    void runAsyncShouldNotRunTaskBecauseTaskStatusIsInterrupted() {
        TaskDto taskDto = TestData.getTaskDto(TaskStatus.INTERRUPTED);
        cseValidAdapterService.runAsync(taskDto);
        verify(cseValidClient, never()).run(any());
        verify(streamBridge, never()).send(any(), any());
    }
    ...
    @Test
    void runAsyncShouldNotRunTaskBecauseTaskStatusIsStopping() {
        TaskDto taskDto = TestData.getTaskDto(TaskStatus.STOPPING);
        cseValidAdapterService.runAsync(taskDto);
        verify(cseValidClient, never()).run(any());
        verify(streamBridge, never()).send(any(), any());
    }
